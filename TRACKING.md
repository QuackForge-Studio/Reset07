...
- [x] **Loop events shipped** (feature/loop-events merged): seeded 1-2 in-loop side events via `src/play/data/events.ts` (`pickLoopEvents(seed, reachable, last?)` — mulberry32, deterministic; supply = hold-interact crate → heat reset + 40% overdrive; ambush = trigger zone → 0.8s telegraph → 2 drones + 1 detonator → clear all → +30% overdrive). HUD side chips via `sideObjectiveSnapshot`. 71 unit tests (11 new events tests), probe 11/11 E2E PASS, smoke + narrow green.
